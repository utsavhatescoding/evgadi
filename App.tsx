...
export default function Home() {
  ...
  const [marketVehicleData, setMarketVehicleData] = useState<MarketVehicle[]>(marketVehicles);
  ...
  const catalogBrands = useMemo(
    () => ["All brands", ...Array.from(new Set(marketVehicleData.map((item) => item.brand))).sort()],
    [marketVehicleData],
  );
  const catalogVehicles = useMemo(() => {
    const query = catalogSearch.trim().toLowerCase();
    return marketVehicleData.filter((item) =>
      (catalogBrand === "All brands" || item.brand === catalogBrand) &&
      (!query || `${item.brand} ${item.model} ${item.variants || ""}`.toLowerCase().includes(query)),
    );
  }, [catalogBrand, catalogSearch, marketVehicleData]);

  useEffect(() => {
    if (!isSupabaseConfigured) return;
    loadCatalogueVehicles<MarketVehicle>()
      .then((records) => {
        if (records.length) setMarketVehicleData(records);
      })
      .catch(() => {});
              ...
              <article className="catalogue-card" key={vehicle.id}>
                ...
                <div className="catalogue-card-body">
                  ...
                  <div className="catalogue-specs">
                    <span><b>{vehicle.batteryKwh || "—"}</b> {vehicle.batteryKwh ? "kWh" : "battery"}</span>
                    <span><b>{vehicle.rangeKm || "—"}</b> {vehicle.rangeKm ? `km ${vehicle.rangeStandard || ""}` : "range"}</span>
                    <span><b>{vehicle.clearanceMm || "—"}</b> {vehicle.clearanceMm ? "mm clearance" : "clearance"}</span>
                  </div>
          ...
          <section className="vehicle-detail-modal" role="dialog" aria-modal="true" aria-labelledby="vehicle-detail-title"
            ...
            <div className="detail-content">
              ...
              <div className="detail-facts">
                <div><small>Battery</small><strong>{selectedMarketVehicle.batteryKwh ? `${selectedMarketVehicle.batteryKwh} kWh` : "Pending"}</strong></div>
                <div><small>Claimed range</small><strong>{selectedMarketVehicle.rangeKm ? `${selectedMarketVehicle.rangeKm} km ${selectedMarketVehicle.rangeStandard || ""}` : "Pending"}</strong></div>
                <div><small>Ground clearance</small><strong>{selectedMarketVehicle.clearanceMm ? `${selectedMarketVehicle.clearanceMm} mm` : "Not confirmed"}</strong></div>
              </div>
              <div className="detail-spec-list">
                <div><span>Battery chemistry</span><strong>{selectedMarketVehicle.batteryType || "Not published"}</strong></div>
                <div><span>AC charging</span><strong>{selectedMarketVehicle.acCharging || "Not published"}</strong></div>
                <div><span>DC charging</span><strong>{selectedMarketVehicle.dcCharging || "Not published"}</strong></div>
                <div><span>Vehicle warranty</span><strong>{selectedMarketVehicle.vehicleWarranty || "Not published"}</strong></div>
                <div><span>Battery warranty</span><strong>{selectedMarketVehicle.batteryWarranty || "Not separately confirmed"}</strong></div>
                <div><span>Authorised distributor</span><strong>{selectedMarketVehicle.distributor || "Verification pending"}</strong></div>
              </div>
              <div className="detail-evidence">
                <Icon name="shield" />
                <div>
                  <strong>{selectedMarketVehicle.sourceTier === "official-source-linked" ? "Official source linked" : "Market-directory record"}</strong>
                  <span>Checked {selectedMarketVehicle.checkedAt || "date pending"}. Unknown values remain unfilled.</span>
                  {selectedMarketVehicle.sourceUrl && <a href={selectedMarketVehicle.sourceUrl} target="_blank" rel="noreferrer">View supporting source ↗</a>}
                </div>
              </div>
